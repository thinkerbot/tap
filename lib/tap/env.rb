  ...
  class Env
    ...
    class << self  
      ...
      def instantiate(path)
        ...
        instances[path] ||= Env.new(read_config(path), Root.new( File.dirname(path) ))
      end
      ...
      def path_config(key, value=[]) 
        ...
        config_attr(key, value) do |input|
          ...
          instance_variable_set(instance_variable, input.collect {|path| root[path]}.uniq)
        end
      end
    ...
    config_attr :env_paths, [] do |value|
      check_paths_configurable
      
      @env_paths = value.collect {|path| root[path] }.uniq
      @envs = env_paths.collect {|path| Env.new_by_env_path(path) }
    end
...
# manifest(:generators, :generator_paths, ["lib/generators"]) do
#   generator_paths.reverse_each do |generator_path|
...
#       generators[generator] = {:path => path}
#     end
#   end
# end
