...
class WorkflowTest < Test::Unit::TestCase
  ...
  def test_define_task_sets_task_in_instance_variable_by_name
    ...
    t1 = w.tap_task
    t2 = w.tap_task('alt')
    assert_equal({:tap_task => t1, 'alt' => t2}, w.instance_variable_get(:@tap_task))
  end
  ...
  def test_defined_tasks_utilize_configurations_by_the_same_name
    w = Define.new(:tap_task => {:key => 'value'})
  ...
  def test_initialization_of_a_task_using_non_hash_or_nil_configs_raises_error
    w = Define.new :int => 2, :str => 'str', :hash => {}, :nil => nil
  ...
  def test_initialization_initializes_class_using_block
    ...
    t = w.task_with_block
    assert_equal Tap::Task, t.class
    assert_equal Define::BLOCK, t.task_block
  ...
  def test_define_task_setter_sets_instance_variable_if_hash
    w = Define.new
    w.tap_task = {:key => 'value'}
    assert_equal({:key => 'value'}, w.instance_variable_get(:@tap_task))
  end
  
  def test_define_task_setter_sets_input_by_name_in_instane_variable_if_input_is_not_a_hash
    w = Define.new
    w.tap_task = 'value'
    assert_equal({:tap_task => 'value'}, w.instance_variable_get(:@tap_task))
    assert_equal 'value', w.tap_task
  end
