...
class ConstantManifestTest < Test::Unit::TestCase
  ...
  def test_register_globs_for_files_under_dir_and_adds_these_to_search_paths
    a = method_tempfile('a.txt') {}
    b = method_tempfile('b.txt') {}
    c = method_tempfile('c.rb') {}

    m.register(method_root[:output], "*.txt")
    assert_equal [[method_root[:output], [a,b]]], m.search_paths
    
    m.register(method_root[:output], "*.rb")
    assert_equal [
      [method_root[:output], [a,b]],
      [method_root[:output], [c]]
    ], m.search_paths
  end
  ...
  def test_build_searches_for_constants_with_const_attr_along_paths
    a = method_tempfile('a.txt') {|file|  file << "# A::attr" }
    b = method_tempfile('b.txt') {|file|  file << "# B::attr\n# Nested::B::attr" }
    c = method_tempfile('c.txt') {|file|  file << "# C::another" }
  ...
  def test_build_infers_default_const_name_from_relative_filepath
    a = method_root.filepath(:output, 'a.txt') 
    b = method_root.filepath(:output, 'path/to/b.txt')
    
    FileUtils.mkdir_p(File.dirname(b))
    File.open(a, 'w') {|file|  file << "# ::attr" }
    File.open(b, 'w') {|file|  file << "# ::attr" }
  ...
  def test_each_stores_new_entries_before_yielding_to_block
    a = method_tempfile('a.txt') {|file|  file << "# A::attr" }
    b = method_tempfile('b.txt') {|file|  file << "# B::attr\n# Nested::B::attr" }
    c = method_tempfile('c.txt') {|file|  file << "# C::attr" }
    m.register(method_root[:output], "*.txt")
