...
class SyncMergeTest < Test::Unit::TestCase
  ...
  def test_splat_sync_merge
    ...
    t1_0 = Tracer.new(1, runlist) do |task, input|
      input.collect {|str| task.mark(str) }
    end 
    t2_0 = Tracer.new(2, runlist) do |task, *inputs|
      inputs.collect {|str| task.mark(str) }
    ...
    t2_0.sync_merge(t0_0, t1_0, :splat => true)
    t0_0.enq(['a', 'b'])
    t1_0.enq(['a', 'b'])
    app.run
    ...
    m1_0a = [[nil, ["a", "b"]], [t1_0, ["a 1.0", "b 1.0"]], [0, "a 1.0"]]
    m1_0b = [[nil, ["a", "b"]], [t1_0, ["a 1.0", "b 1.0"]], [1, "b 1.0"]]
    
    assert_audits_equal([
      [[m0_0a, m0_0b, m1_0a, m1_0b], [t2_0, ['a 0.0 2.0', 'b 0.0 2.0', 'a 1.0 2.0', 'b 1.0 2.0']]],
    ], app._results(t2_0))
  end
